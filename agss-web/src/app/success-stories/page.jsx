...
export default function SuccessStoriesPage() {
            ...
            <div className="h-72 md:h-96 w-full relative">
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-slate-900 via-slate-900/60 to-transparent flex flex-col justify-end p-8 md:p-12">
                <div className="flex flex-wrap gap-2 mb-4">
                  {data.tags.map((tag) => (
                    <span key={tag} className="px-3 py-1 bg-white/20 backdrop-blur-md text-white text-[11px] font-bold uppercase tracking-wider rounded-full">
                      {tag}
                    </span>
                  ))}
                </div>
            ...
            <div className="p-8 md:p-12 grid grid-cols-1 lg:grid-cols-3 gap-12">
              ...
              <div className="lg:col-span-1">
                <div className="bg-white p-8 rounded-[2rem] border border-slate-200/60 shadow-lg sticky top-32">
                  <h4 className="font-bold text-slate-900 mb-6 text-xl">Case Overview</h4>
                  <div className="space-y-6 mb-8">
                    ...
                    <div className="flex items-start gap-4">
                      <div className="w-12 h-12 rounded-xl flex items-center justify-center bg-slate-50 text-slate-500 shrink-0"><Target size={24} strokeWidth={2} /></div>
                      <div>
                    ...
                    <div className="w-full h-px bg-slate-100"></div>
                    <Link href="/contact" className="flex items-center justify-center gap-2 w-full py-4 bg-slate-900 hover:bg-slate-800 text-white font-bold rounded-xl shadow-lg mt-2 transition-all group">
  ...
  return (
    // แก้ไข: นำ overflow-hidden ออกจาก Wrapper เพื่อให้สามารถเลื่อนจอ (Scroll) ลงไปได้
            ...
            return (
              ...
              >
                ...
                <div className="absolute inset-0 p-6 md:p-8 flex flex-col justify-end">
                  {/* Category / Subtitle */}
                  <span className={`text-[10px] font-bold uppercase tracking-widest text-cyan-400 mb-2 transform translate-y-2 group-hover:translate-y-0 transition-transform duration-300`}>
                    {item.subtitle}
                  </span>
                  
                  {/* Title */}
                  <h3 className={`font-bold text-white leading-tight drop-shadow-md transition-all duration-300 ${index === 0 ? 'text-2xl md:text-3xl mb-2' : 'text-xl mb-0 group-hover:mb-2'}`}>
                    {item.title}
                  </h3>
                  
                  {/* Summary (Slides up on hover for smaller cards, always visible on big card) */}
                  <p className={`text-slate-300 text-sm font-light leading-relaxed line-clamp-2 transition-all duration-500 ${index === 0 ? 'opacity-100' : 'opacity-0 h-0 group-hover:h-auto group-hover:opacity-100 mt-1'}`}>
                    {item.summary}
                  </p>
                </div>
              </div>
